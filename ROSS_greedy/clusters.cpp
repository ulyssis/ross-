...
int numCR = 100;
	...
	PoolofNeighborhoods Pool_Groups_trueindex(1000,Neighbors());
	PoolofNeighborhoods Pool_MotherGroups(1000,Neighbors());	//所有nodes的mother group的集合, 包括那些只有一个mother group的node
	...
	Cluster NodesAfterSearch(numCR,0);
	Cluster overlapping_nodes;
	Cluster real_overlapping_nodes;

	std::vector<int> Nodesvisited(numCR,0);
...
void Clusters::ClusteringPhaseI(){
	...
	for(unsigned int i=0;i<NumOfNodes;i++)	//traversal nodes for clusterhead!
		...
		Cluster Neighbor=Pool_Neighbors[i];	//node i's neighborhood, members are represeted by index No.

		for(unsigned int j=0;j<Neighbor.size();j++)	//get into neighborhood of node i, to check whether it is clusterhead.
							    //judge formation of clustering via comparment of ai an bi.
		    {
		...
		//This is the easiest situation to judge a basin node
		if(!x)	//a_i is smaller than all its neighbors, node i is basin node!
		    {
	...
	//对于Pool_Groups中的每一个成员，不能包含已知的cluster heads
		...
		//the other situation to judge a basin node
		else if(!y)	//a_i is smaller and equal to its neighboring nodes.
		    {
...
//找出了node i周围有相同ai的点！ 但是还得看，这些点是不是具备了成为head的可能，也就是“周围点的a_i没有比它的a_i大的”！！
			...
			if(!z)	//z=o means node bi_i>=bi_neighbors
			    {
			    std::cout<<"node "<<i<<" is basin node and clusterhead although certain neighboring nodes having identical ai"<<"\n";
...
unsigned int Clusters::CheckVisitComplete()		//return the number of visited nodes!
    {
    //phaseI 的遍历遍数
	Num++;
	int n=0;
...
/*
	//outpu nodes within groups, make sure there is no node which is not included into a cluster1
...
//		{
//		sum_1=sum_1 + Pool_Groups_trueindex[i].size();
//		}
...
//	    std::cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<"\n";
//	    std::cout<<"the block is to test the validity of transformation of clusters: "<<"\n";
//	    std::cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<"\n";
//	    //std::cout<<"\n";
//	for(unsigned int i=0; i<Pool_Groups_trueindex.size(); i++)
//	    {
//	    if(Pool_Groups_trueindex[i].size()!=0)
//		{
//		for(unsigned int j=0; j<Pool_Groups_trueindex[i].size(); j++)
//		    {
//		    for (unsigned int k=0; k<c.size(); k++)
//			{
//			if(c[k]==Pool_Groups_trueindex[i][j])
//			    {
//			    //Pool_Groups_trueindex[i] contains c[k], in other words, Pool_Groups_trueindex[i] is cluster head c[k]'s cluster
//
//			    std::cout<<"Group("<<c[k]<<")= "<<"\t";
//			    for(unsigned int l=0; l<Pool_Groups_trueindex[i].size(); l++)
//				{
//				std::cout<< Pool_Groups_trueindex[i][l] <<"\t";
//				}
//			    std::cout<<"\n";
//			    }
//			}
...
//	for (unsigned int i=0; i<Pool_Groups_trueindex.size(); i++)
//		{
//		sum_0=sum_0 + Pool_Groups_trueindex[i].size();
//		}
...
//////////////////////////////////////////////////////////////
//	Pool_Groups_trueindex --> Pool_Groups
//
// if Pool_Groups_trueindex[i].size()!=0, then, this vector should replace the counterpart in Pool_Groups.

//unsigned int sum=0;
//for (unsigned int i=0; i<10;i++)
//	{
//	sum=sum+Pool_Groups_trueindex[i].size();
//	}
	...
	if(Pool_Groups_trueindex[i].size()!=0)
		...
		if(i==c[j])
		    {
//		    Pool_Groups[j].clear();
		    Pool_Groups[j]=Pool_Groups_trueindex[i];
		    }
		}
	    }
	}
...
//    for (unsigned int i=0; i<Pool_Groups_trueindex.size();i++)
//    	{
//	Pool_Groups_trueindex[i].clear();
//    	}



    std::cout<<"\n";
    std::cout<<"From here is the new results: "<<"\n";
    ...
    std::cout<<"After update of cluster membership, the number of all nodes involved is "<<NumOfAllNodes<<"\n";
...
//output #CCC and #OCC!
	...
	for(unsigned int j=0; j<Pool_Groups[i].size(); j++)	//Pool_Groups[i][j] is a member of group i, we want to find the number of ccc between it and other nodes!
	    ...
	    //this restriction is cancled, becasue cluster node can also connect with other node, not only border nodes!
...
//
    ...
    average_CCC = SumNumCCC/Pool_Groups.size();
