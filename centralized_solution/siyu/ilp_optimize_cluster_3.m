function ilp_optimize_cluster_3
...
% for size of 3, paras for size of 2 is accordingly decided.
para = [0.7, 0.8, 0.9];

for p = 1:length(para)
    
    alph = [para(p), 1 - para(p)];
    ...
    for k = 1 : length(file_list)
        ...
        non_active_node_cost = max_cost * 1000; % hard code cost
        non_active_cluster_cost = max_cost*100 *ones(1,num_cluster); % hard code cost
        ...
        fprintf(fid, '(%f, %f, %d, %d) \n', channel_per_cluster,channel_per_cluster_2_3, good_cluster_3(k),good_cluster_2(k) );
        
    end
    
    
    fprintf(fid, '(%f, %f, %f, %f) \n', sum(channel_per_cluster_all)/length(file_list),sum(channel_per_cluster_2_3_all)/length(file_list), sum(good_cluster_3)/length(good_cluster_3),sum(good_cluster_2)/length(good_cluster_2) );
    fclose(fid);
end
end
